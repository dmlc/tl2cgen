...
public class Predictor implements Serializable, KryoSerializable {
   ...
   * @throws TL2cgenError error during loading the shared lib
   */
  public Predictor(String libpath, int nthread, boolean verbose) throws TL2cgenError {
    this.num_thread = nthread;
    this.verbose = verbose;
    initNativeLibrary(libpath);
  }

  private void initNativeLibrary(String libpath) throws TL2cgenError {
    ...
    if (f.isDirectory()) {  // libpath is a diectory
      ...
      if (!lib_found) {
        throw new TL2cgenError(String.format(
    ...
    TL2cgenJNI.checkCall(TL2cgenJNI.TL2cgenPredictorLoad(
            this.libpath, this.num_thread, long_out));
    handle = long_out[0];

    // Fetch meta information from model
    TL2cgenJNI.checkCall(TL2cgenJNI.TL2cgenPredictorQueryNumClass(
            handle, long_out));
    num_class = (int) long_out[0];
    TL2cgenJNI.checkCall(TL2cgenJNI.TL2cgenPredictorQueryNumFeature(
            handle, long_out));
    num_feature = (int) long_out[0];
    String[] str_out = new String[1];
    TL2cgenJNI.checkCall(TL2cgenJNI.TL2cgenPredictorQueryPredTransform(
            handle, str_out));
    pred_transform = str_out[0];
    float[] fp_out = new float[1];
    TL2cgenJNI.checkCall(TL2cgenJNI.TL2cgenPredictorQuerySigmoidAlpha(
            handle, fp_out));
    sigmoid_alpha = fp_out[0];
    TL2cgenJNI.checkCall(TL2cgenJNI.TL2cgenPredictorQueryRatioC(
            handle, fp_out));
    ratio_c = fp_out[0];
    TL2cgenJNI.checkCall(TL2cgenJNI.TL2cgenPredictorQueryGlobalBias(
            handle, fp_out));
  ...
  public INDArray predict(DMatrix batch, boolean verbose, boolean pred_margin)
      throws TL2cgenError {
    long[] out = new long[1];
    TL2cgenJNI.checkCall(TL2cgenJNI.TL2cgenPredictorQueryResultSize(
        this.handle, batch.getHandle(), out));
    int result_size = (int)out[0];

    String[] s_out = new String[1];
    TL2cgenJNI.checkCall(TL2cgenJNI.TL2cgenPredictorQueryLeafOutputType(this.handle, s_out));
    String leaf_output_type = s_out[0];
    switch (leaf_output_type) {
      case "float32": {
        float[] out_result = new float[result_size];
        if (num_thread == 1) {
          TL2cgenJNI.checkCall(TL2cgenJNI.TL2cgenPredictorPredictBatchWithFloat32Out(
      ...
      case "float64": {
        ...
        if (num_thread == 1) {
          TL2cgenJNI.checkCall(TL2cgenJNI.TL2cgenPredictorPredictBatchWithFloat64Out(
              this.handle, batch.getHandle(), verbose, pred_margin, out_result, out));
        } else {
          synchronized (this) {
            TL2cgenJNI.checkCall(TL2cgenJNI.TL2cgenPredictorPredictBatchWithFloat64Out(
      ...
      case "uint32": {
        ...
        if (num_thread == 1) {
          TL2cgenJNI.checkCall(TL2cgenJNI.TL2cgenPredictorPredictBatchWithUInt32Out(
              this.handle, batch.getHandle(), verbose, pred_margin, out_result, out));
        } else {
          synchronized (this) {
            TL2cgenJNI.checkCall(TL2cgenJNI.TL2cgenPredictorPredictBatchWithUInt32Out(
      ...
      default:
        throw new TL2cgenError("Unknown leaf output type: " + leaf_output_type);
    }
  }
  ...
  public synchronized void dispose() {
    if (handle != 0L) {
      TL2cgenJNI.TL2cgenPredictorFree(handle);
      handle = 0;
    }
  }
