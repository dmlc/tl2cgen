...
static void scan_thresholds(ASTNode* node, std::vector<bool>* is_categorical) {
  CategoricalConditionNode* cat_cond = dynamic_cast<CategoricalConditionNode*>(node);
  if (cat_cond) {
...
std::vector<bool> ASTBuilder<ThresholdType, LeafOutputType>::GenerateIsCategoricalArray() {
