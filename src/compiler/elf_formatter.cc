...
unsigned int const SHF_X86_64_LARGE = 0x10000000;

char const ident_str[EI_NIDENT] = {
...
void AppendToBuffer(std::vector<char>* dest, void const* src, std::size_t count) {
...
void FormatArrayAsELF(std::vector<char>* elf_buffer) {
  ...
  char const comment[] = "\0GCC: (Ubuntu 7.4.0-1ubuntu1~18.04.1) 7.4.0\0\0\0\0";
  ...
  char const strtab[] = "\0arrays.c\0nodes";
  static_assert(sizeof(strtab) == 16, ".strtab has incorrect size");

  /* Format section name table */
  char const shstrtab[]
      = "\0.symtab\0.strtab\0.shstrtab\0.text\0.data\0.bss\0.lrodata\0.comment\0"
        ".note.GNU-stack\0\0";
  ...
  const std::size_t e_shoff = sizeof(elf_header) + array_size + sizeof(comment) + sizeof(symtab)
                              + sizeof(strtab) + sizeof(shstrtab);
...
void AllocateELFHeader(std::vector<char>* elf_buffer) {
  TL2CGEN_LOG(FATAL) << "dump_array_as_elf is not supported in non-Linux OSes";
}
