...
public class DMatrix {
   ...
   * @throws TL2cgenError error during matrix construction
   */
  public DMatrix(float[] data, int[] col_ind, long[] row_ptr, long num_row, long num_col)
      throws TL2cgenError {
    long[] out = new long[1];
    TL2cgenJNI.checkCall(TL2cgenJNI.TL2cgenDMatrixCreateFromCSRWithFloat32In(
  ...
  public DMatrix(double[] data, int[] col_ind, long[] row_ptr, long num_row, long num_col)
      throws TL2cgenError {
    long[] out = new long[1];
    TL2cgenJNI.checkCall(TL2cgenJNI.TL2cgenDMatrixCreateFromCSRWithFloat64In(
        data, col_ind, row_ptr, num_row, num_col, out));
    this.handle = out[0];
    setDims();
  }
  ...
  public DMatrix(float[] data, float missing_value, long num_row, long num_col)
      throws TL2cgenError {
    long[] out = new long[1];
    TL2cgenJNI.checkCall(TL2cgenJNI.TL2cgenDMatrixCreateFromMatWithFloat32In(
  ...
  public DMatrix(double[] data, double missing_value, long num_row, long num_col)
      throws TL2cgenError {
    long[] out = new long[1];
    TL2cgenJNI.checkCall(TL2cgenJNI.TL2cgenDMatrixCreateFromMatWithFloat64In(
        data, num_row, num_col, missing_value, out));
    this.handle = out[0];
    setDims();
  }

  private void setDims() throws TL2cgenError {
    long[] out_num_row = new long[1];
    long[] out_num_col = new long[1];
    long[] out_num_elem = new long[1];
    TL2cgenJNI.checkCall(TL2cgenJNI.TL2cgenDMatrixGetDimension(
  ...
  public synchronized void dispose() {
    if (this.handle != 0L) {
      TL2cgenJNI.TL2cgenDMatrixFree(this.handle);
      this.handle = 0;
    }
  }
}
