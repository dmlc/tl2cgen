...
#include <tl2cgen/detail/compiler/elf_formatter.h>
...
char const* const header_template = R"TL2CGENTEMPLATE(
#include <stdlib.h>
#include <string.h>
#include <math.h>
#include <stdint.h>
...
char const* const main_template = R"TL2CGENTEMPLATE(
#include "header.h"
...
char const* const return_multiclass_template =
    R"TL2CGENTEMPLATE(
...
char const* const return_template =
    R"TL2CGENTEMPLATE(
...
char const* const arrays_template = R"TL2CGENTEMPLATE(
#include "header.h"
...
inline std::pair<std::string, std::string> FormatNodesArray(
    treelite::ModelImpl<float, float> const& model) {
  util::ArrayFormatter nodes(100, 2);
  util::ArrayFormatter nodes_row_ptr(100, 2);
  int node_count = 0;
  nodes_row_ptr << "0";
  for (auto const& tree : model.trees) {
    for (int nid = 0; nid < tree.num_nodes; ++nid) {
      if (tree.IsLeaf(nid)) {
        TL2CGEN_CHECK(!tree.HasLeafVector(nid))
            << "multi-class random forest classifier is not supported in FailSafeCompiler";
        nodes << fmt::format("{{ 0x{sindex:X}, {info}, {cleft}, {cright} }}", "sindex"_a = 0,
            "info"_a = util::ToStringHighPrecision(tree.LeafValue(nid)), "cleft"_a = -1,
            "cright"_a = -1);
      } else {
        TL2CGEN_CHECK(tree.SplitType(nid) == treelite::SplitFeatureType::kNumerical
        ...
        nodes << fmt::format("{{ 0x{sindex:X}, {info}, {cleft}, {cright} }}",
            ...
            "info"_a = util::ToStringHighPrecision(tree.Threshold(nid)),
            "cleft"_a = tree.LeftChild(nid), "cright"_a = tree.RightChild(nid));
      }
    }
...
inline std::pair<std::vector<char>, std::string> FormatNodesArrayELF(
  ...
  elf::AllocateELFHeader(&nodes_elf);
  ...
  elf::FormatArrayAsELF(&nodes_elf);

  return std::make_pair(
      nodes_elf, fmt::format("const int nodes_row_ptr[] = {{\n{}\n}};", nodes_row_ptr.str()));
}

// Get the comparison op used in the tree ensemble model
// If splits have more than one op, throw an error
inline std::string GetCommonOp(treelite::ModelImpl<float, float> const& model) {
  ...
  TL2CGEN_CHECK_EQ(ops.size(), 1) << "FailSafeCompiler only supports models where all splits use "
...
class FailSafeCompilerImpl {
 public:
  explicit FailSafeCompilerImpl(CompilerParam const& param) : param_(param) {}

  CompiledModel Compile(treelite::Model const& model_ptr) {
    TL2CGEN_CHECK(model_ptr.GetThresholdType() == treelite::TypeInfo::kFloat32
                  && model_ptr.GetLeafOutputType() == treelite::TypeInfo::kFloat32)
        << "Failsafe compiler only supports models with float32 thresholds and float32 leaf "
           "outputs";
    ...
    TL2CGEN_CHECK(!model.average_tree_output)
        << "Averaging tree output is not supported in FailSafeCompiler";
    TL2CGEN_CHECK(model.task_type == treelite::TaskType::kBinaryClfRegr
                  || model.task_type == treelite::TaskType::kMultiClfGrovePerClass)
        << "Model task type unsupported by FailSafeCompiler";
    TL2CGEN_CHECK_EQ(model.task_param.leaf_vector_size, 1)
        << "Model with leaf vectors is not supported by FailSafeCompiler";
    pred_tranform_func_ = PredTransformFunction(model_ptr);
    files_.clear();

    char const* predict_function_signature
        = (num_class_ > 1)
    ...
    std::string accumulator_definition
        = (num_class_ > 1
                ? fmt::format("float sum[{num_class}] = {{0.0f}}", "num_class"_a = num_class_)
                : std::string("float sum = 0.0f"));

    std::string output_statement
        = (num_class_ > 1 ? fmt::format("sum[tree_id % {num_class}] += tree[nid].info.leaf_value;",
               "num_class"_a = num_class_)
                          : std::string("sum += tree[nid].info.leaf_value;"));

    std::string return_statement
        = (num_class_ > 1
                ? fmt::format(return_multiclass_template, "num_class"_a = num_class_,
                    "global_bias"_a = util::ToStringHighPrecision(model.param.global_bias))
    ...
    if (param_.dump_array_as_elf > 0) {
      if (param_.verbose > 0) {
        TL2CGEN_LOG(INFO) << "Dumping arrays as an ELF relocatable object...";
      }
    ...
    const treelite::ModelParam model_param = model.param;
    const std::string query_functions_definition
        = fmt::format(templates::query_functions_definition_template, "num_class"_a = num_class_,
            "num_feature"_a = num_feature_, "pred_transform"_a = model_param.pred_transform,
            "sigmoid_alpha"_a = model_param.sigmoid_alpha, "ratio_c"_a = model_param.ratio_c,
            "global_bias"_a = model_param.global_bias,
            "threshold_type_str"_a = treelite::TypeInfoToString(treelite::TypeToInfo<float>()),
            "leaf_output_type_str"_a = treelite::TypeInfoToString(treelite::TypeToInfo<float>()));

    main_program << fmt::format(main_template, "nodes_row_ptr"_a = nodes_row_ptr,
        ...
        "num_tree"_a = model.trees.size(), "compare_op"_a = GetCommonOp(model),
        "accumulator_definition"_a = accumulator_definition,
        "output_statement"_a = output_statement, "return_statement"_a = return_statement);
    ...
    if (param_.dump_array_as_elf > 0) {
      ...
      files_["arrays.c"]
          = CompiledModel::FileEntry(fmt::format(arrays_template, "nodes"_a = nodes));
    }
    ...
    files_["header.h"]
        = CompiledModel::FileEntry(fmt::format(header_template, "dllexport"_a = DLLEXPORT_KEYWORD,
...

    {
      ...
      if (!extra_file_list.empty()) {
        writer.Key("extra");
        writer.StartArray();
        for (auto const& extra_file : extra_file_list) {
...
FailSafeCompiler::FailSafeCompiler(CompilerParam const& param)
    : pimpl_(std::make_unique<FailSafeCompilerImpl>(param)) {
  if (param.verbose > 0) {
    TL2CGEN_LOG(INFO) << "Using FailSafeCompiler";
  }
  if (param.annotate_in != "NULL") {
    TL2CGEN_LOG(INFO) << "Warning: 'annotate_in' parameter is not applicable for "
                         "FailSafeCompiler";
  }
  if (param.quantize > 0) {
    TL2CGEN_LOG(INFO) << "Warning: 'quantize' parameter is not applicable for "
                         "FailSafeCompiler";
  }
  if (param.parallel_comp > 0) {
    TL2CGEN_LOG(INFO) << "Warning: 'parallel_comp' parameter is not applicable for "
                         "FailSafeCompiler";
  }
  if (std::isfinite(param.code_folding_req)) {
    TL2CGEN_LOG(INFO) << "Warning: 'code_folding_req' parameter is not applicable "
...
CompiledModel FailSafeCompiler::Compile(treelite::Model const& model) {
  return pimpl_->Compile(model);
}

CompilerParam FailSafeCompiler::QueryParam() const {
