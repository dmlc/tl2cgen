...
void ASTBuilder<ThresholdType, LeafOutputType>::Split(int parallel_comp) {
  if (parallel_comp <= 0) {
  ...
  for (ASTNode* node : top_ac_node->children) {
    TL2CGEN_CHECK(dynamic_cast<ConditionNode*>(node)
  ...
  int const nunit = parallel_comp;
  int const unit_size = (ntree + nunit - 1) / nunit;
  std::vector<ASTNode*> tu_list;  // list of translation units
  int const current_num_tu = count_tu_nodes(this->main_node);
  for (int unit_id = 0; unit_id < nunit; ++unit_id) {
    ...
    if (tree_begin < tree_end) {
      TranslationUnitNode* tu = AddNode<TranslationUnitNode>(top_ac_node, current_num_tu + unit_id);
