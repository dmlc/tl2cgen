 ...
 public:
  DenseDMatrix()
      : data_{},
        ...
        num_row_{0},
        num_col_{0} {}
  DenseDMatrix(std::vector<ElementType> data, ElementType missing_value, std::uint64_t num_row,
      std::uint64_t num_col)
      : data_{std::move(data)},
...
class CSRDMatrix {
 public:
  ...
  CSRDMatrix(std::vector<ElementType> data, std::vector<std::uint32_t> col_ind,
      std::vector<std::uint64_t> row_ptr, std::uint64_t num_row, std::uint64_t num_col)
      : data_{std::move(data)},
        col_ind_{std::move(col_ind)},
        row_ptr_{std::move(row_ptr)},
        num_row_{num_row},
        num_col_{num_col} {}
  CSRDMatrix(void const* data, std::uint32_t const* col_ind, std::uint64_t const* row_ptr,
      std::uint64_t num_row, std::uint64_t num_col)
      : num_row_{num_row}, num_col_{num_col} {
    auto const* data_ptr = static_cast<ElementType const*>(data);
    std::uint64_t const num_elem = row_ptr[num_row];
    data_ = std::vector<ElementType>{data_ptr, data_ptr + num_elem};
    col_ind_ = std::vector<std::uint32_t>{col_ind, col_ind + num_elem};
    row_ptr_ = std::vector<std::uint64_t>{row_ptr, row_ptr + num_row + 1};
  }
  CSRDMatrix(void const*, void const*, std::uint64_t, std::uint64_t) {
    TL2CGEN_LOG(FATAL) << "Invalid set of arguments";
  }
  std::uint64_t GetNumRow() const {
    return num_row_;
