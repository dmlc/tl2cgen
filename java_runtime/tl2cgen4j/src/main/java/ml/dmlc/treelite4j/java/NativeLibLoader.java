...
/**
 * Class to load the native lib ``libtl2cgen4j.so``. Normally, users
 * do not have to worry about this class, since ``mvn package`` automatically
 * bundles ``libtl2cgen4j.so`` into the JAR file. However, the method
...
public class NativeLibLoader {
  ...
  private static final String[] libNames = new String[]{"tl2cgen4j"};

  /**
   * Initialization method to load the native tl2cgen4j lib at startup
   * @throws IOException when tl2cgen4j lib is not found
   */
  static synchronized void initTL2cgenRuntime() throws IOException {
    if (!initialized) {
