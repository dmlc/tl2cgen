...
bool fold_code(ASTNode* node, CodeFoldingContext* context,
  ...
  if ((node->data_count && !std::isnan(context->log_root_data_count)
          && context->log_root_data_count - std::log(*node->data_count) >= context->magnitude_req)
      || (node->sum_hess && !std::isnan(context->log_root_sum_hess)
          && context->log_root_sum_hess - std::log(*node->sum_hess) >= context->magnitude_req)) {
    ...
    TL2CGEN_CHECK_NE(node_loc, -1);  // parent should have a link to current node
    parent_node->children[node_loc] = context->create_new_translation_unit ? tu_node : folder_node;
...
template <typename ThresholdType, typename LeafOutputType>
bool ASTBuilder<ThresholdType, LeafOutputType>::FoldCode(
    double magnitude_req, bool create_new_translation_unit) {
  CodeFoldingContext context{magnitude_req, std::numeric_limits<double>::quiet_NaN(),
      std::numeric_limits<double>::quiet_NaN(), create_new_translation_unit,
