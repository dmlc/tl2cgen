...
* **Host machine** : the machine running TL2cgen.
* **Target machine** : the machine on which predictions will be made. The host
  machine may or may not be identical to the target machine.
* **Shared library** : a blob of executable functions that can be imported by
  ...
  TL2cgen will produce a shared library containing the prediction function
...
Option 1: Deploy prediction code with the TL2cgen package
=========================================================
If feasible, this option is probably the most convenient. On the target machine, install
TL2cgen by running pip:
...
-----------------------
\1. On the host machine, install TL2cgen and import your tree ensemble model.
...
.. code-block:: python
  ...
  import treelite
  model = treelite.Model.load("your_model.json", model_format="xgboost_json")
  # You may also use `from_xgboost` method or the builder class

\2. Export your model as a **source package** by calling the method
:py:func:`tl2cgen.export_srcpkg`. The source package will contain C code
...
.. code-block:: python
  ...
  tl2cgen.export_srcpkg(model, toolchain=toolchain,
                        pkgpath="./mymodel.zip", libname="mymodel.so")

.. note:: On the value of ``toolchain``

  TL2cgen supports only three toolchain configurations ("msvc", "gcc", "clang")
