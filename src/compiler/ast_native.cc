...
class ASTNativeCompilerImpl {
 public:
  ...
  CompiledModel CompileImpl(treelite::ModelImpl<ThresholdType, LeafOutputType> const& model) {
    ...
    TREELITE_CHECK(model.task_type != treelite::TaskType::kMultiClfCategLeaf)
 ...
 private:
  ...
  template <typename ThresholdType, typename LeafOutputType>
  void WalkAST(ASTNode const* node, std::string const& dest, size_t indent) {
    MainNode const* t1;
    AccumulatorContextNode const* t2;
    ConditionNode const* t3;
    OutputNode<LeafOutputType> const* t4;
    TranslationUnitNode const* t5;
    QuantizerNode<ThresholdType> const* t6;
    CodeFolderNode const* t7;
    if ((t1 = dynamic_cast<MainNode const*>(node))) {
      HandleMainNode<ThresholdType, LeafOutputType>(t1, dest, indent);
    } else if ((t2 = dynamic_cast<AccumulatorContextNode const*>(node))) {
      HandleACNode<ThresholdType, LeafOutputType>(t2, dest, indent);
    } else if ((t3 = dynamic_cast<ConditionNode const*>(node))) {
      HandleCondNode<ThresholdType, LeafOutputType>(t3, dest, indent);
    } else if ((t4 = dynamic_cast<OutputNode<LeafOutputType> const*>(node))) {
      HandleOutputNode<ThresholdType, LeafOutputType>(t4, dest, indent);
    } else if ((t5 = dynamic_cast<TranslationUnitNode const*>(node))) {
      HandleTUNode<ThresholdType, LeafOutputType>(t5, dest, indent);
    } else if ((t6 = dynamic_cast<QuantizerNode<ThresholdType> const*>(node))) {
      HandleQNode<ThresholdType, LeafOutputType>(t6, dest, indent);
    } else if ((t7 = dynamic_cast<CodeFolderNode const*>(node))) {
  ...
  inline void AppendToBuffer(std::string const& dest, std::string const& content, size_t indent) {
  ...
  inline void PrependToBuffer(std::string const& dest, std::string const& content, size_t indent) {
    files_[dest].content = util::IndentMultiLineString(content, indent) + files_[dest].content;
  }

  template <typename ThresholdType, typename LeafOutputType>
  void HandleMainNode(MainNode const* node, std::string const& dest, size_t indent) {
    const std::string threshold_type = native::TypeInfoToCTypeString(TypeToInfo<ThresholdType>());
    ...
    const std::string query_functions_definition = fmt::format(
        native::query_functions_definition_template, "num_class"_a = task_param_.num_class,
    ...
    AppendToBuffer(dest,
        fmt::format(native::main_start_template, "array_is_categorical"_a = array_is_categorical_,
    ...
    const std::string query_functions_prototype = fmt::format(
        native::query_functions_prototype_template, "dllexport"_a = DLLEXPORT_KEYWORD);
    AppendToBuffer("header.h",
        fmt::format(native::header_template, "dllexport"_a = DLLEXPORT_KEYWORD,
            "predict_function_signature"_a = predict_function_signature,
    ...
    if (task_param_.num_class > 1) {
      AppendToBuffer(dest,
          fmt::format(native::main_end_multiclass_template, "num_class"_a = task_param_.num_class,
  ...
  template <typename ThresholdType, typename LeafOutputType>
  void HandleACNode(AccumulatorContextNode const* node, std::string const& dest, size_t indent) {
    const std::string leaf_output_type
        = native::TypeInfoToCTypeString(TypeToInfo<LeafOutputType>());
    if (task_param_.num_class > 1) {
      AppendToBuffer(dest,
  ...
  template <typename ThresholdType, typename LeafOutputType>
  void HandleCondNode(ConditionNode const* node, std::string const& dest, size_t indent) {
    NumericalConditionNode<ThresholdType> const* t;
    std::string condition_with_na_check;
    if ((t = dynamic_cast<NumericalConditionNode<ThresholdType> const*>(node))) {
      /* numerical split */
      ...
      CategoricalConditionNode const* t2 = dynamic_cast<CategoricalConditionNode const*>(node);
      TL2CGEN_CHECK(t2);
  ...
  void HandleOutputNode(
      OutputNode<LeafOutputType> const* node, std::string const& dest, size_t indent) {
    AppendToBuffer(dest, RenderOutputStatement(node), indent);
    TL2CGEN_CHECK_EQ(node->children.size(), 0);
  }

  template <typename ThresholdType, typename LeafOutputType>
  void HandleTUNode(TranslationUnitNode const* node, std::string const& dest, size_t indent) {
  ...
  template <typename ThresholdType, typename LeafOutputType>
  void HandleQNode(
      QuantizerNode<ThresholdType> const* node, std::string const& dest, size_t indent) {
    const std::string threshold_type = native::TypeInfoToCTypeString(TypeToInfo<ThresholdType>());
    ...
    if (!array_threshold.empty() && !array_th_begin.empty() && !array_th_len.empty()) {
      PrependToBuffer(dest,
          fmt::format(native::qnode_template, "total_num_threshold"_a = total_num_threshold,
              "threshold_type"_a = threshold_type),
          0);
      AppendToBuffer(dest,
          fmt::format(native::quantize_loop_template, "num_feature"_a = num_feature_), indent);
    }
  ...
  template <typename ThresholdType, typename LeafOutputType>
  void HandleCodeFolderNode(CodeFolderNode const* node, std::string const& dest, size_t indent) {
    TL2CGEN_CHECK_EQ(node->children.size(), 1);
    ...
    util::RenderCodeFolderArrays<ThresholdType, LeafOutputType>(
        ...
        [this](OutputNode<LeafOutputType> const* node) { return RenderOutputStatement(node); },
    ...
    if (array_nodes.empty()) {
      ...
      AppendToBuffer(dest,
          fmt::format(native::eval_loop_template, "node_array_name"_a = node_array_name,
  ...
  inline std::string ExtractNumericalCondition(NumericalConditionNode<ThresholdType> const* node) {
    const std::string threshold_type = native::TypeInfoToCTypeString(TypeToInfo<ThresholdType>());
    std::string result;
  ...
  inline std::string ExtractCategoricalCondition(CategoricalConditionNode const* node) {
    std::string result;
    std::vector<uint64_t> bitmap = util::GetCategoricalBitmap(node->matching_categories);
  ...
  inline std::string RenderOutputStatement(OutputNode<LeafOutputType> const* node) {
    const std::string leaf_output_type
        = native::TypeInfoToCTypeString(TypeToInfo<LeafOutputType>());
    ...
    if (task_param_.num_class > 1) {
      if (node->is_vector) {
        ...
        TL2CGEN_CHECK_EQ(node->vector.size(), static_cast<size_t>(task_param_.num_class))
